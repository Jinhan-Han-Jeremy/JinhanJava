...
public class DNALinkedList {
	...
	private boolean matches(Node<Character> startNode, String target) {
		...
		if (target.length() == 0)
			return true;
		else if (startNode.getData().equals(target.charAt(0))) {
			return matches(startNode.getNext(), target.substring(1));
		} else
			return false;
	...
	public Node<Character> find(String target) {
		...
		Node<Character> check = head;
		while (check != null) {
			if (matches(check, target)) {
				return check;
			}
			check = check.getNext();
		}
	...
	public void transpose(String transposon, String target) {
		...
		if (firstNodeOfTransposon == null)
			throw new IllegalArgumentException("Cannot find transposon in chromosome.");

		// Find starting node of target.
		Node<Character> firstNodeOfTarget = this.find(target);
		if (firstNodeOfTarget == null)
			throw new IllegalArgumentException("Cannot find target in chromosome.");
